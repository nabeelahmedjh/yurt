...
const pattern = /(\.\.\/)/g;
let serverPathSet = false;

// using relative path, so no need to pass the path as argument
// let contentRootPath = yargs.argv.d;

const currentDirectory = __dirname;

const relativeFolderPath = path.join(currentDirectory, '../../../directory');

console.log('Directory path: ', relativeFolderPath);
let contentRootPath = relativeFolderPath;
...
class AccessRules {
    constructor(path = "/*", role = "member", read = true, write = false, writeContents = false, copy = true, download = true, upload = false, isFile, message = "defaault") {
        this.path = path;
        this.role = role;
        this.read = read;
        this.write = write;
...
function renameFolder(req, res) {
    ...
    if (permission != null && (!permission.read || !permission.write)) {
        ...
        if (checkForDuplicates(contentRootPath + req.body.data[0].filterPath, newName, req.body.data[0].isFile)) {
            ...
            fsExtra.moveSync(oldDirectoryPath, newDirectoryPath);
            (async () => {
                await FileManagerDirectoryContent(req, res, req.body.data[0].isFile ? newDirectoryPath : newDirectoryPath + "/").then(data => {
                    response = { files: data };
                    response = JSON.stringify(response);
                    res.setHeader('Content-Type', 'application/json');
                    res.json(response);
...
app.post('/Upload', multer(multerConfig).any('uploadFiles'), function (req, res) {
    ...
    if (pathPermission != null && (!pathPermission.read || !pathPermission.upload)) {
        ...
        for (var i = 0; i < fileName.length; i++) {
            fsExtra.move('./' + fileName[i], path.join(contentRootPath, req.body.path + fileName[i]), function (err) {
                if (err) throw err;
            });
        }
        res.send('Success');
...
/**
 * Handles the ceating a directory for server request
 */
app.post('/CreateServerDirectory', function (req, res) {
    // creating a foulder for a server with server ID

    let serverID = req.body.serverID;
    let serverPath = path.join(__dirname, 'directory', 'servers', serverID);
...
app.post('/', function (req, res) {
    ...
    function getRules() {
        ...
        if (req.query.role) {
            ...
            // Permissions in case user is a admin
            if (req.query.role == "admin") {
                accessRules.push(new AccessRules("/*", "member", Permission.Allow, Permission.Allow, Permission.Allow, Permission.Allow, Permission.Allow, Permission.Allow, false, "You are a admin"));
                accessRules.push(new AccessRules("/*.*", "member", Permission.Allow, Permission.Allow, Permission.Allow, Permission.Allow, Permission.Allow, Permission.Allow, true, "You are a admin"))
            }
            // Permissions in case user is a member
            if (req.query.role == "member") {
                accessRules.push(new AccessRules("/*", "member", Permission.Allow, Permission.Deny, Permission.Deny, Permission.Deny, Permission.Allow, Permission.Deny, false, "You don't have Permission for this action"));
                accessRules.push(new AccessRules("/*.*", "member", Permission.Allow, Permission.Deny, Permission.Deny, Permission.Deny, Permission.Allow, Permission.Deny, true, "You don't have Permission for this action"))
            }
    ...
    if (req.body.action == "read") {
        ...
        if (req.query.rootPath && req.query.rootPath.split("/").length == 3) {
            let requestedRelativePath = req.query.rootPath.split('/')
            let originalContentRootPath = contentRootPath.split('/')
            // let baseDirector = contentRootPath.split("/")[-1]
            // console.log(contentRootPath)

            console.log(requestedRelativePath)
            console.log(originalContentRootPath)
            let currentPath = originalContentRootPath[originalContentRootPath.length - 1]
            let requestedPath = requestedRelativePath[requestedRelativePath.length - 1]
            console.log("Current path: " + currentPath);
            console.log("Requested path: " + requestedPath);

            if (currentPath === 'directory') {
                contentRootPath = contentRootPath + req.query.rootPath;
            }

            if (currentPath !== requestedPath && currentPath !== 'directory' && originalContentRootPath[originalContentRootPath.length - 2] == 'servers') {
                originalContentRootPath[originalContentRootPath.length - 1] = requestedPath;
                contentRootPath = originalContentRootPath.join('/');

            }
            // if (!serverPathSet || currentServer !== requestedServer) {
            //     console.log("Setting server path")
            //     contentRootPath = + '/directory/' + "servers" + requestedServer
            //     serverPathSet = true;
